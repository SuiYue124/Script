...
async function sendNotify(
  ...
  if (text.includes('cookie已失效')) {
    const fs = require('fs');
    const TOUCH_FILE = 'wskey.lastrunning';
    let lastRunningMs = 0;
    if (fs.existsSync(TOUCH_FILE)) lastRunningMs = fs.statSync(TOUCH_FILE).mtimeMs;
    const elapsed = (Date.now() - lastRunningMs) / 1000;
    const MIN_SECONDS_ELAPSED = 30; // s
    if (elapsed < MIN_SECONDS_ELAPSED) return;

    const exec = require('child_process').execSync;
    const output = exec(`task Zy143L_wskey/wskey.py; touch ${TOUCH_FILE}`);
    desp += '\n\n自动更新cookie\n' + output;
  }
  //提供6种通知
