...
function buildwenjian() {
  cd ${GITHUB_WORKSPACE}
  if [[ ! -d ${GITHUB_WORKSPACE}/build ]]; then
    rm -rf bendi && git clone https://github.com/GWen124/AutoBuild-OpenWrt ${GITHUB_WORKSPACE}/AutoBuild-OpenWrt
    judge "build文件下载"
    cp -Rf ${GITHUB_WORKSPACE}/AutoBuild-OpenWrt/build ${GITHUB_WORKSPACE}/build
    [[ -f ${Builb} ]] && cp -Rf ${GITHUB_WORKSPACE}/build/* ${Builb}/
    rm -rf ${GITHUB_WORKSPACE}/AutoBuild-OpenWrt
  fi
}

function bianyi_xuanxiang() {
  cd ${GITHUB_WORKSPACE}
  [[ ! -d ${GITHUB_WORKSPACE}/build ]] && buildwenjian
  source ${GITHUB_WORKSPACE}/build/${firmware}/settings.ini
  if [[ "${EVERY_INQUIRY}" == "true" ]]; then
    ECHOY "请用WinSCP工具连接ubuntu，然后在 build/${firmware} 里面设置好自定义文件"
  ...
  echo
  echo
  source ${GITHUB_WORKSPACE}/build/${firmware}/settings.ini > /dev/null 2>&1
...
function feeds_clean() {
  ...
  rm -rf "${Home}"/GWen124 && git clone --depth 1 -b "${REPO_BRANCH}" https://github.com/GWen124/OpenWrt-Software "${Home}"/GWen124
  judge "插件包下载"
  cp -Rf "${Home}"/GWen124/* "${Home}" && rm -rf "${Home}"/GWen124
  ...
  cp -Rf ${GITHUB_WORKSPACE}/build/* "${Builb}"
...
function op_jiaoben() {
  cd ${GITHUB_WORKSPACE}
  rm -rf AutoBuild-OpenWrt && git clone https://github.com/GWen124/AutoBuild-OpenWrt
  judge "编译脚本下载"
  cp -Rf ${GITHUB_WORKSPACE}/build/* AutoBuild-OpenWrt/build
  chmod -R +x AutoBuild-OpenWrt/build && cp -Rf AutoBuild-OpenWrt/build ${Home}
...
function op_feeds_update() {
  ...
  cp -rf ${GITHUB_WORKSPACE}/build/${firmware}/${CONFIG_FILE} ${Home}/.config
}
...
function make_defconfig() {
  ...
  make defconfig
  ./scripts/diffconfig.sh > ${GITHUB_WORKSPACE}/build/${config_bf}
...
function op_make() {
  ...
  if [[ ${firmware} == "Mortal_source" ]] || [[ "${firmware}" == "Tianling_source" ]]; then
    if [[ `ls -a ${COMFIRMWARE} | grep -c "immortalwrt"` == '0' ]]; then
      ...
      ECHOY "友情提示：因为编译失败，如若${CONFIG_FILE}配置有更改，请把build/${config_bf}内容复制,然后覆盖到build/${firmware}/${CONFIG_FILE}更新保存配置"
      sleep 1
      exit 1
    fi
  else
  ...
  cp -Rf ${COMFIRMWARE}/config.buildinfo ${GITHUB_WORKSPACE}/build/${firmware}/${CONFIG_FILE}
}
