...
SUCCESS() {
    ...
    echo -e "默认后台密码：${GREEN_COLOR}alist${RES}"
	...
	echo -e "卸载：${GREEN_COLOR}curl -fsSL "https://nn.ci/alist.sh" | bash -s uninstall${RES}"
...
UNINSTALL() {
    ...
    rm -rf $INSTALL_PATH /lib/systemd/system/alist.service
    systemctl daemon-reload
